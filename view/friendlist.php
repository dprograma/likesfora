        ...
        <div class="container-fluid" style="margin-top: 7.5rem;">
            <div class="row">
                <div class="col-md-8 col-12 p-4">
                    ...
                    <table class="table">
                        <?php
                        $fid = "'" . implode(',', $friendsid) . "'";
                        $sql = "SELECT DISTINCT userId, email, password, firstname, lastname, username, dob, sex, phone, profileimage, coverimage, description, bio, loggedin, registered FROM user WHERE `userid` NOT IN (SELECT `friendsid` FROM friends WHERE `userid` = '$userid') AND `userid` != '$userid' ";
                        $stma = $mysqli->query($sql);
                        if ($stma != null) {
                            // $stma->bind_param(str_repeat('i', (count($friendsid))), ...array_values($friendsid));
                            // $stma->execute();
                            // $rst = $stma->get_result();
                            while ($row = $stma->fetch_assoc()) {
                                !empty($row['profileimage']) ? $img = $row['profileimage'] : $img = 'avater.png';
